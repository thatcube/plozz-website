import fs from 'fs';
const list=await (await fetch('http://localhost:9341/json/list')).json();
...
await send('Page.navigate',{url}); await new Promise(r=>setTimeout(r,2000));
await send('Runtime.evaluate',{expression:'window.scrollTo(0,document.body.scrollHeight)'}); await new Promise(r=>setTimeout(r,1400));
